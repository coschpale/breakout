...
screen.onkey(key='Up', fun=paddle.move_left(True))
screen.onkey(key='Right', fun=paddle.move_right)
screen.onkey(key='Down', fun=paddle.move_right(True))
...
def check_collision_with_paddle():
	...
	ball_x = ball.xcor()
	# print("Ball = " + str(ball_x) + " Paddle = " + str(paddle_x))
	...
	if ball.distance(paddle) < 110 and ball.ycor() < -250:
		...
		if ball_x < paddle_x + 110 and ball_x >= paddle_x + 70:
			ball.bounce_from_paddle(x_factor=2)
			return
			
		elif ball_x < paddle_x + 70 and ball_x >= paddle_x + 25:
			ball.bounce_from_paddle(x_factor=1)
			return
			
		elif ball_x < paddle_x + 25 and ball_x >= paddle_x - 25:
			ball.bounce_from_paddle(x_factor=0)
			return
			
		elif ball_x < paddle_x -25 and ball_x >= paddle_x - 70:
			ball.bounce_from_paddle(x_factor=-1)
			return
			
		elif ball_x < paddle_x -70 and ball_x > paddle_x - 110:
			ball.bounce_from_paddle(x_factor=-2)
			return
